...
<div class="migration-schemas-container">
@foreach($columns as $column_index => $column)
  @php
      $modifier_configuration_for_colummn_type = $selectable_column_types[$column['column_type']]['configs'] ?? [];
  @endphp
  <div wire:key="column-definition-{{ $column_index }}" class="col-md-12 well migration-schema row m-1 p-2 pl-3 ps-3">
    ...
    <div class="form-group col-md-3 mb-1 px-1">
      ...
      <input  type="text"
              ...
              wire:model.blur="columns.{{ $column_index }}.column_name"
      ...
      <select
        ...
        @foreach($selectable_column_types_order as $label => $column_type)
          @if(is_array($column_type))
            <optgroup label="{{ ucfirst($label) }}" wire:key="opt-group-{{$label}}-{{$column_index}}">
              @foreach($column_type as $value)
              <option value="{{ $value }}" {{ $this->isColumnTypeEnabled($value, $column) ? '' : 'disabled' }} wire:key="option-{{$label}}-{{$column_index}}-{{$value}}">{{ ucfirst($value) }}</option>
              @endforeach
            </optgroup>
          @else
    ...
    @if (isset($columns[$column_index]['show_modifiers']) && $columns[$column_index]['show_modifiers'] != true)
      <div class="form-group col-md-2 mb-1 px-1 ">
        ...
        <button class="d-block mt-1 ml-1 ms-1 btn btn-sm btn-link"
                type="button"
                href="#"
  ...
  @if(!$loop->last && in_array($columns[$column_index+1]['column_type'], ['timestamps', 'timestampsTz']))
    <button type="button" wire:click.prevent="addColumn" class="btn btn-primary ml-1 ms-1 mt-3 mb-3 add-migration-schema-button">+ Add Column</button>
  @elseif($loop->last && 
...
@push('after_scripts')
  ...
  window.addEventListener('confirmColumnDelete', (e) => {
      ...
      if (e.detail[0].callback === 'removeColumnAndRelationship') {
      ...
      @this[e.detail[0].callback](...e.detail[0].argv)
  });
