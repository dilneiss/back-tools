...
class RelationshipSchema extends Component
{
    ...
    public $creating_model_relation = false;
    ...
    {
        // we don't need to bust the cache here because the cache had already been bust by
        // MigrationSchema component, but we still need to call it.
        $this->interactsWithApplicationInitialization();
    ...
    public function addRelationship()
    {
        ...
        if ($column) {
            $this->dispatch(
                ...
                relationship_index: count($this->relationships) - 1
            )->to(MigrationSchema::class);
        }
    }
    ...
    {
        if (! empty($created_model) && $this->creating_model_relation !== false) {
    ...
    {
        $this->dispatch('confirmRelationshipDelete', compact('callback', 'argv'));
    }
}
