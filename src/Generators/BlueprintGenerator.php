...
class BlueprintGenerator implements GeneratorInterface
{
    ...
    public function generate($request)
    {
        ...
        switch ($operation) {
            ...
            case 'model':
                ...
                break;
            case 'operation':
                return app(OperationGenerator::class)->generate($request);
                break;
            default:
    ...
    private function createAndRunYamlFile($request, $model)
    {
        ...
        // modify migration
        $idColumn = collect($request->input('columns'))
            ->filter(function ($column) {
                return $column['column_type'] === 'id' || $column['column_type'] === 'uuid';
            })
            ->map(function ($column) {
        ...
        // modify the migration file to account for custom keys and provide a way for `unique` rule to work on update.
        if (! empty($this->createdFiles['migrations'])) {
            foreach ($this->createdFiles['migrations'] as $migration) {
                // modify migration in case the id column name is changed
                if ($idColumn && $idColumn !== 'id') {
                    $result = Str::of(File::get(base_path($migration)))->replaceFirst('->id()', "->id('$idColumn')");
                    File::put(base_path($migration), $result);
                }

                if ($request->boolean('run_migration')) {
                    $this->registerFiles($migration);
        ...
        if (! empty($this->createdFiles['models'])) {
            foreach ($this->createdFiles['models'] as $generatedModel) {
                ...
                // if the table id is different from the default 'id', $primaryKey should be supplied on the model
                if ($idColumn && $idColumn !== 'id') {
                    $modelPath = app_path("Models/{$name}.php");

                    if (File::exists($modelPath)) {
                        $result = Str::of(File::get($modelPath))
                            ->replaceLast('}', "\n    protected \$primaryKey = '$idColumn';\n}")
                            ->replaceLast("'id'", "'$idColumn'");
                        File::put($modelPath, $result);
                    }
                }

                if ($request->boolean('build_crud')) {
                    Artisan::call('backpack:crud', ['name' => $name, '--no-interaction' => true]);
                    ...
                    $columns = collect($request->input('columns'))
                        ...
                        ->map(function ($column) use ($context, $relationshipNameMap, $idColumn) {
                            ...
                            if (in_array('nullable', $modifiers) && ! in_array('nullable', $rules)) {
                            ...
                            // customize blueprint rules
                            if (in_array('unique', $modifiers)) {
                                if ($idColumn) {
                                    return "'$name' => '$rules,'.\$this->$idColumn.',$idColumn',";
                                }
                            }
    ...
    public function createYamlFile($model, $request)
    {
        ...
        if (count(array_intersect(['id', 'uuid'], array_column($columns, 'column_type'))) === 0) {
            $content->push('    id: false');
        }
        ...
        foreach ($columns as $column) {
            ...
            if ($type === 'foreignId') {
                ...
                if ($relation_table) {
                    ...
                    // when creating a foreign type column, blueprint automatically generate the relation
                    // we should exclude it from relationships, avoiding double creation.
                    $relationships = $relationships->filter(function ($item) use ($name) {
                        return $item['relationship_column'] !== $name;
                    });
                } else {
